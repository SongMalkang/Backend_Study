...
export const SHA256 = (s: string): string => {
  ...
  const Ch = (x: any, y: any, z: any) => {
    return (x & y) ^ (~x & z);
  };
  const Maj = (x: any, y: any, z: any) => {
    return (x & y) ^ (x & z) ^ (y & z);
  };
  const Sigma0256 = (x: any) => {
    return S(x, 2) ^ S(x, 13) ^ S(x, 22);
  };
  const Sigma1256 = (x: any) => {
    return S(x, 6) ^ S(x, 11) ^ S(x, 25);
  };
  const Gamma0256 = (x: any) => {
    return S(x, 7) ^ S(x, 18) ^ R(x, 3);
  };
  const Gamma1256 = (x: any) => {
    return S(x, 17) ^ S(x, 19) ^ R(x, 10);
  };

  const coreSha256 = (m: number[], l: any): any => {
    const str2binbResult = m;
    const length = l;
    ...
    str2binbResult[length >> 5] |= 0x80 << (24 - (length % 32));
    str2binbResult[(((length + 64) >> 9) << 4) + 15] = length;
  ...
  const Utf8Encode = (string: string): string => {
    const blankRemoveString = string.replace(/\r\n/g, '\n');
    let utftext = '';

    for (let n = 0; n < blankRemoveString.length; n += 1) {
      const c = blankRemoveString.charCodeAt(n);
  ...
  const encodeS = Utf8Encode(s);
  return binb2hex(coreSha256(str2binb(encodeS), s.length * chrsz));
};
